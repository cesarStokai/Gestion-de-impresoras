...
class _RequisicionesPageState extends ConsumerState<RequisicionesPage> {
        ...
        data: (list) => ListView.separated(
          ...
          itemBuilder: (_,i){
            ...
            return ListTile(
              ...
              subtitle: Text('Pedido: ${r.fechaPedido.toLocal().toShortIsoDate()}\nEstado: ${r.estado}'),
              trailing: IconButton(
                icon: const Icon(Icons.delete, color: Colors.red),
                onPressed: ()=> ref.read(requisicionesDaoProvider).deleteById(r.id!),
              ),
            );
          },
        ),
  ...
  void _showForm([Requisicione? r]) {
    final isNew = r==null;
    final fechaP = TextEditingController(text: r?.fechaPedido.toIso8601String() ?? DateTime.now().toIso8601String());
    final fechaE = TextEditingController(text: r?.fechaEstimEntrega.toIso8601String() ?? '');
        ...
        content: StatefulBuilder(builder:(ctx,set){
          ...
          return Column(mainAxisSize: MainAxisSize.min, children:[
            DropdownButtonFormField<int>(
              ...
              items: toners.map((t)=> DropdownMenuItem(
                value: t.id!, child: Text('${t.color} (${t.impresoraId})')
              )).toList(),
            ...
            TextField(
              ...
              onTap: () async {
                final d = await showDatePicker(
                  context: context,
                  initialDate: fechaE.text.isEmpty?DateTime.now():DateTime.parse(fechaE.text),
                  firstDate: DateTime(2000), lastDate: DateTime(2100),
                );
            ...
            DropdownButtonFormField<String>(
              ...
              items: _reqStates.map((s)=>DropdownMenuItem(value:s,child:Text(s))).toList(),
        ...
        actions:[
          ...
          ElevatedButton(
            onPressed: tonerId!=null?(){
              ...
              if(isNew){
                dao.insertOne(RequisicionesCompanion.insert(
                ...
                dao.updateOne(Insertable<Requisicione>.fromData({
                  'id': r!.id!,
                  'tonere_id': tonerId,
                  'fecha_pedido': DateTime.parse(fechaP.text),
                  'fecha_estim_entrega': DateTime.parse(fechaE.text),
                  'estado': estado,
